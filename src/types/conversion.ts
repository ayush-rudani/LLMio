...
import type { UIMessage as LegacyUIMessage, ToolInvocation, Message as V4Message } from "ai-legacy"
...
// Tool conversion
function convertV4ToolInvocationToV5ToolUIPart(toolInvocation: ToolInvocation): ToolUIPart {
    return {
...
function buildPartsFromTopLevelFields(msg: V4Message): MyUIMessage["parts"] {
    ...
    if (msg.toolInvocations) {
        parts.push(...msg.toolInvocations.map(convertV4ToolInvocationToV5ToolUIPart))
    }
...
export function convertV4MessageToV5(msg: V4Message | MyUIMessage, index: number): MyUIMessage {
    ...
    const parts = msg.parts ? convertPartsArray(msg.parts) : buildPartsFromTopLevelFields(msg)

    return { ...base, parts }
}

// V5 to V4 conversion
function convertV5ToolUIPartToV4ToolInvocation(part: ToolUIPart): ToolInvocation {
    const state =
...
export function convertV5MessageToV4(msg: MyUIMessage): LegacyUIMessage {
    ...
    for (const part of msg.parts) {
        if (part.type === "text") {
            ...
            const toolInvocation = convertV5ToolUIPartToV4ToolInvocation(part as ToolUIPart)
